...
)
{
    ...
    for(auto i=0; i<TOTAL_ITER; i++)
    {
        ...
        {
            I_TYPE sum = infx1[j]+infx2[j];
            ac_int<2,true> sign = sum < 0 ? -1 : 1;
            I_TYPE abs_sum = sign*sum;
            I_TYPE shifted_abs_sum = abs_sum + relu_bias[i*PE+j];

            I_TYPE magnitude = shifted_abs_sum > 0 ? shifted_abs_sum : 0;
            magnitude = magnitude > 6 ? 6 : magnitude;
            
            act[j] = sign*magnitude;;
            state_buffer[i*PE+j] = act[j];
            // std::cout << state_buffer[i*PE+j] << '\n';
        }
        ac_int<out_stream_width> act_packed = *reinterpret_cast<ac_int<out_stream_width,false>*>(act);
        output_stream.write(act_packed);
